...
const crc32 = (r) => {for(var a,o=[],c=0;c<256;c++){a=c;for(var f=0;f<8;f++)a=1&a?3988292384^a>>>1:a>>>1;o[c]=a}for(var n=-1,t=0;t<r.length;t++)n=n>>>8^o[255&(n^r.charCodeAt(t))];return(-1^n)>>>0}; // eslint-disable-line

const crc32hex = (x) => crc32(x).toString(16)
...
*/
const hashLine = (onLeft,filePath,fileRevision,lineNumber) => {
    const temp = `${onLeft?"left":"right"}${filePath}${fileRevision}${lineNumber.toString()}`
    return crc32hex(temp)
}

const hashHunkSeparator = (onLeft,filePath,fileRevision,hunkContent) => {
    const temp = `${onLeft?"left":"right"}${filePath}${fileRevision}${hunkContent}`
    return crc32hex(temp)
}

const hashFileHeader = (onLeft,filePath,fileRevision) => {
...
const invertHex = (hex) => [...hex].map(digit => inverse[digit]).join("")
